...
class BasePage:
    ...
    def find_select(self, locator: "Locator") -> Select:
        ...
        """
        return Select(self.wait.until(ec.presence_of_element_located(locator)))
    ...
    def wait_for_text_in_element(self, locator: "Locator", text: str, timeout: int = _DEFAULT_TIMEOUT) -> None:
        """
        Espera que el elemento definido por locator tenga un texto en el atributo value. Es decir, si tengo
        <EtiquetaHTML value="un_valor">ASDASD</EtiquetaHTML> este método va a esperar que "ASDASD" esté
        contenido en value
        ...
        :return: None
        """
        self.get_wait(timeout).until(ec.text_to_be_present_in_element(locator, text),
                                     message=f"Esperaba que el elemento tuviera el texto {text},"
                                             f"pero no lo tenía")
    ...
    def value_complete(self, locator, value):
        """
        Se utiliza para cuando estricamente se necesita que el value se coloque entero ya que a veces escribe muy rapido
        los datos y no los termina de completar

        :param locator: objeto que abstrae el elemento a esperar
        :param value: Texto a asignarle al atributo value del elemento
        :return: None
        """
        __element = self.find_element(locator)
        __element.send_keys(value)
        len_value = len(str(value))
        while len_value != len(str(__element.get_attribute('value'))):
            __element.send_keys(value[len(__element.get_attribute('value')):len_value])

    def wait_for_css_class_in_element(self, locator, css):
        """
        Espera que el elemento definido por locator tenga la clase indicada en css
        :param locator:  locator del elemento
        :param css: clase
        :return: no tiene
        """
        self.wait.until(css_class_in_element(locator.ret_locator, css))

    def wait_for_css_class_to_disappear(self, locator, css):
        """
        Espera que el elemento definido por locator NO tenga la clase indicada en css
        :param locator: locator del elemento
        :param css: clase
        :return: no tiene
        """
        self.wait_for_css_class_in_element(locator, css)
        self.wait.until_not(css_class_in_element(locator.ret_locator, css))

    def scroll_to_element(self, locator: "Locator"):
        __element = self.driver.find_element(*locator.ret_locator)
        self.driver.execute_script('arguments[0].scrollIntoView(true);', __element)
...
class css_class_in_element(object):
    """
    Clase para implementar un wait personalizado
    """
